    ...
    <div class="main-area">
      <div class="top-wrapper" @touchmove.stop>
        <div class="top-area">
          ...
          <div class="site-info">
            <h1 class="site-name">Yuan's Blog</h1>
            <h1 class="site-desc">Coding!</h1>
          </div>
        </div>
        <div class="sidebar-menus">
          ...
          <ul class="nav-menu">
            ...
            <li class="nav-dropdown-container" v-for="category_level1 in articleCategoryList" :key="category_level1.id">
              ...
              <a class="nav-link" :href="'/articles/category/' + category_level1.id">{{category_level1.name}}<span
                class="arrow"></span> </a>
              <ul class="nav-dropdown">
                <li v-for="category_level2 in category_level1.children" :key="category_level2.id">
                  <iv-icon type="minus-round"></iv-icon>
                  <a class="nav-link" :href="'/artocles/category/' + category_level2.id">{{ category_level2.name}}</a>
                  <ul class="nav-dropdown">
                    <li v-for="category_level3 in category_level2.children" :key="category_level3.id">
                      <iv-icon type="minus-round"></iv-icon>
                      <a class="nav-link" :href="'/articles/category/' + category_level3.id">{{ category_level3.name
                        }}</a>
                    </li>
                  </ul>
                </li>
        ...
        <div class="sidebar-menus">
          ...
          <ul class="nav-menu">
            ...
            <li class="nav-dropdown-container" v-for="category_level1 in bookCategoryList" :key="category_level1.id">
              <iv-icon type="minus-round"></iv-icon>
              <a class="nav-link" :href="'/books/category/' + category_level1.id">{{ category_level1.name }}<span class="arrow"></span></a>
              <ul class="nav-dropdown">
                <li v-for="category_level2 in category_level1.children" :key="category_level2.id">
                  <iv-icon type="minus-round"></iv-icon>
