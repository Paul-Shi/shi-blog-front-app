...
export default {
      ...
      let recursiveCategory = function (categorys, selectCategoryId) {
        ...
        for (let index = 0; index < categorys.length; index++) {
          ...
          if (category.id === selectCategoryId) {
            ...
            if (result) {
              // 如果在sub_category中找到了，则需要将自己的id也记录，组成一条树路径
              recursiveCategorys.push(category)
              recursiveCategoryIds.push(category.id)
              return result
            }
  ...
  watch: {
    ...
    selectRecursiveCategorys: function (newSelectRecursiveCategorys) {
      // 更新样式
      this.$nextTick(() => {
        if (newSelectRecursiveCategorys.length === 0) {
          // 表示没有默认选择
        ...
        newSelectRecursiveCategorys.map((id) => {
